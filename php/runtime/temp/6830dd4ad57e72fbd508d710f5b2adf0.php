<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:72:"E:\wamp64\www\qwdz\public/../application/admin\view\store\editstore.html";i:1531894409;s:60:"E:\wamp64\www\qwdz\application\admin\view\common\header.html";i:1529373481;s:58:"E:\wamp64\www\qwdz\application\admin\view\common\left.html";i:1531894263;}*/ ?>
<!DOCTYPE html>
<!--
...
<div class="page-content">
    ...
    <div class="page-breadcrumbs">
        <ul class="breadcrumb">
            ...
            <li class="active">区域门店设置</li>
            <li>
                <i class="fa fa-list"></i>
                <a href="<?php echo url('store/index'); ?>">门店管理</a>
            </li>
            <li class="active">修改门店</li>
        </ul>
    </div>
    <!-- /Page Breadcrumb -->
    <!-- Page Header -->
            ...
            <div class="widget">
                ...
                <div class="widget-body">
                    <div id="horizontal-form">
                        <form id="submit" class="form-horizontal" role="form" enctype="multipart/form-data" ><!--action="<?php echo url('store/editstore'); ?>" method="post">-->
                            <input type="hidden" name="id" value="<?php echo $store['id']; ?>">
                            <div class="form-group">
                                <label class="col-sm-2 control-label no-padding-right">地区</label>
                                <div class="col-sm-6">
                                    <select id="pid" onchange="findCity()">
                                        <?php if(is_array($city) || $city instanceof \think\Collection || $city instanceof \think\Paginator): $i = 0; $__LIST__ = $city;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$to): $mod = ($i % 2 );++$i;if($citydata['pname'] == $to['name']): ?>
                                        <option selected="selected" value="<?php echo $to['id']; ?>"><?php echo $to['name']; ?></option>
                                        <?php else: ?>
                                        <option value="<?php echo $to['id']; ?>"><?php echo $to['name']; ?></option>
                                        <?php endif; endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                    <select id="cid" name="cid">
                                        <?php if(is_array($citynames) || $citynames instanceof \think\Collection || $citynames instanceof \think\Paginator): $i = 0; $__LIST__ = $citynames;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;if($citydata['name'] == $vo['name']): ?>
                                        <option selected="selected" value="<?php echo $vo['id']; ?>"><?php echo $vo['name']; ?></option>
                                        <?php else: ?>
                            ...
                            <div class="form-group">
                                <label for="name" class="col-sm-2 control-label no-padding-right">门店名称</label>
                                <div class="col-sm-6">
                                    <input class="form-control" id="name"  name="name"  type="text" value="<?php echo $store['name']; ?>">
                                </div>
                                <p class="help-block col-sm-4 red">* 必填</p>
                            </div>
                            <div class="form-group">
                                <label for="tel" class="col-sm-2 control-label no-padding-right">联系电话</label>
                                <div class="col-sm-6">
                                    <input class="form-control" id="tel"  name="tel"  type="text" value="<?php echo $store['tel']; ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="time" class="col-sm-2 control-label no-padding-right">联系时间</label>
                                <div class="col-sm-6">
                                    <input class="form-control" id="time"  name="time"  type="text" value="<?php echo $store['time']; ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="address" class="col-sm-2 control-label no-padding-right">详细地址</label>
                                <div class="col-sm-6">
                                    <input class="form-control" id="address"  name="address"  type="text" value="<?php echo $store['address']; ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label no-padding-right">门店logo</label>
                                <div class="col-sm-6">
                                    <img height="200" width="200" src="<?php echo $store['logo']; ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="logo" class="col-sm-2 control-label no-padding-right">重传logo</label>
                                <div class="col-sm-6">
...
<script>
    ...
    $('#btn').click(function () {
        var url = "<?php echo url('store/editstore'); ?>";
    ...
    function findCity(){
        var pid=$("#pid option:selected").attr("value");
        var url = "<?php echo url('store/findcity'); ?>";
        $.ajax({
            type : "post",
            url  : url,
            data : {"pid" : pid},
            success : function(data){
                ...
                for(i in cityList){
                    $("#id").append("<option value ="+cityList[i].cid+">"+cityList[i].cname+"</option>");
                }*/
                var jsondata = eval(data);
                for(var i=0; i<jsondata.length; i++){
                    $("#cid").append("<option value="+jsondata[i].id+" >"+jsondata[i].name+"</option>");
                }
            }
        });
    }
